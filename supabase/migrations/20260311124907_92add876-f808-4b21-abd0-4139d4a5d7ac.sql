-- Fix shopping list RPC: use base_servings instead of servings (which is always 1)
-- Also add user_household age-based portions support
...
BEGIN
  ...
  ELSE
    -- Check user_household for age-based total_portions
    ...
    IF v_household_total IS NOT NULL AND v_household_total > 0 THEN
      v_effective_portions := v_household_total;
    ELSE
  ...
  WITH menu_items AS (
    SELECT uwmi.recipe_id, 
           v_effective_portions / GREATEST(COALESCE(r.base_servings, 4), 1) as computed_pf,
    ...
    CASE
      ...
      WHEN d.display_unit = 'piece' THEN
        CASE
          ...
          ELSE ROUND(d.display_qty,0)::text || ' ' || d.display_name
        END
      ELSE
        CASE
          WHEN d.display_qty <= 0.5 THEN '½ ' || d.display_name
          WHEN d.display_qty <= 1 THEN '1 ' || d.display_name
          ELSE ROUND(d.display_qty,0)::text || ' ' || d.display_name
        END
    END as formatted
